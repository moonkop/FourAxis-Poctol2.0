...
void Set_Pwm(void)
{
	...
	//装配给各电机的pwm值
	moto_pwm[0] = remoteControl[1]+Roll_core_out-Pitch_core_out+Yaw_core_out;
	moto_pwm[1] = remoteControl[1]+Roll_core_out+Pitch_core_out-Yaw_core_out;
	moto_pwm[2] = remoteControl[1]-Roll_core_out+Pitch_core_out+Yaw_core_out;
	moto_pwm[3] = remoteControl[1]-Roll_core_out-Pitch_core_out-Yaw_core_out;
	...
	Moto_PwmRflash(moto_pwm[0],moto_pwm[1],moto_pwm[2],moto_pwm[3]);
}
...
{
	if(dataPID_Last == dataPID)     //表明四轴已经和遥控器失去连接
	{					
